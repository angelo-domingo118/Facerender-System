...
<script>
    // Set DEBUG flag to control console logging
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        window.addEventListener('fabricjs:object-selected', function(event) {
            ...
            if (obj) {
                ...
                // Update Livewire component with dimensions
                ...
                // Also dispatch selection event to Livewire
                @this.selectLayer(obj.id);
                
                if (DEBUG_LOGS) {
                    console.log('DIMENSION DEBUG: After update, checking component properties');
                    console.log('DIMENSION DEBUG: Current width in component:', @this.get('width'));
                    console.log('DIMENSION DEBUG: Current height in component:', @this.get('height'));
                }
                
                // Update the position display in DOM
        ...
        window.addEventListener('fabricjs:object-modified', function(event) {
            ...
            if (obj) {
                ...
                // Dispatch position update to Livewire
        ...
        // Function to update position display in DOM
        function updatePositionDisplay(obj) {
            if (!obj) return;
            
            // Update position display in DOM if element exists
            ...
            if (dimensionDisplay) {
                ...
                const xValue = dimensionDisplay.querySelector('.x-value');
                const yValue = dimensionDisplay.querySelector('.y-value');
                
                if (xValue) xValue.textContent = Math.round(obj.left || 0);
                if (yValue) yValue.textContent = Math.round(obj.top || 0);
            }
            
            // Update width/height inputs if they exist
            const widthInput = document.querySelector('input[name="width"]');
            if (widthInput) {
                widthInput.value = Math.round(obj.width * obj.scaleX);
                if (DEBUG_LOGS) {
            ...
            if (heightInput) {
                heightInput.value = Math.round(obj.height * obj.scaleY);
                if (DEBUG_LOGS) {
